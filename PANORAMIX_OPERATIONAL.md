...
4. **Opt-in lab loopback:** `PANORAMIX_RUNTIME_ROOT` (fail closed if unset/missing) injects `LabReserveTemporalHook` and subprocesses `python3 -m runtime.apply reserve-temporal` locally. Not guest→ctl HTTP. Not #70 / #78 Done.
...
`sos.runtime_hook.InertRuntimeHandoffHook` is inert: admit/cancel/status/pause/resume/progress/events no-op and the in-process stub is the fallback. The default hook stays inert. Cancel always attempts the hook first, then local cancel. Do **not** invent guest→ctl HTTP. Opt-in lab: `PANORAMIX_RUNTIME_ROOT` (usable panoramix-runtime checkout) injects `LabReserveTemporalHook` on the same seam and subprocesses `python3 -m runtime.apply reserve-temporal` locally — fail closed if unset or the root is missing. Guest `GET /progress` prefers `hook.progress()` when durable-backed. Guest `GET /events` prefers `hook.events()` JSONL when durable-backed. Does **not** close #70 / #78; cloud stays locked.
...
| Invent guest-callable ctl HTTP / mesh destinations / `PLATFORM_RAY_*` for loopback | **Rejected** — no guest→ctl HTTP; no new mesh destinations |
