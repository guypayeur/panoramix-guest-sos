...
- `GET /v0/jobs/{id}/events` — prefers durable reserve-temporal JSONL when a runtime hook provides it (`source: "durable"`, `events_durable: true` / `events_n` when present). Otherwise process-memory fallback (`source: "memory"`). Day-one local durable trail — **not** a SIEM, **not** iec `/v1/audit/events`, **not** a regulatory audit product. Operator/ctl: `python3 -m runtime.apply reserve-temporal events --id cw_…`. Also on the job resource when the hook overlay is present.
...
- Failed/canceled `GET /v0/jobs/{id}` — `terminal` (status + message + optional last events / stage; **not** a SIEM) and `recoverability` (handoff + payload for operator/ctl re-admit; cancel/fail does **not** auto-retry).
