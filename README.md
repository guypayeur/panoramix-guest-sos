...
- Failed/canceled `GET /v0/jobs/{id}` — `terminal` (status + message + optional last events / stage / `next_action` / `valuation` / `stage_name` when the hook supplies them; **omit when missing**; **not** a SIEM) and `recoverability` (handoff + payload for operator/ctl re-admit; one-click `POST /v0/jobs/{id}/re-admit` when a durable hook is active; cancel/fail does **not** auto-retry). Cancel of an already-canceled job is **409** `already_canceled`; succeeded/failed cancel stays **409** `already_terminal`.
...
- `POST /v0/jobs/{id}/pause` / `POST /v0/jobs/{id}/resume` — durable path only. Stub-only (`local.backed == "stub"` or no runtime ref) returns **409** `stub_only`. Illegal transitions (pause when not `running`, resume when not `paused`/`held`, terminal) return **409**. iec-local same-job mid-flight pause is refused unless the hook supplies `can_pause` (runtime tip `d9b9948+` `pause_limit`). When ctl/hook returns `pause_signaled` / `resume_signaled` after pause/resume (runtime tip `e2f41fd+`), those fields pass through on job/progress — **omit when missing**; do **not** invent `held` or enable pause without `can_pause`. Cancel is not pause.

Operator UI Pause/Resume controls are **disabled** for stub jobs (with an explanation pointing at ctl). Durable-backed jobs can pause (`paused`) and resume (`running`, including from `held`) when honest. Same-job iec-local keeps those controls omitted unless ctl supplies them. A brief **signaled** cue appears when ctl returns `pause_signaled` / `resume_signaled` — not invented `held`. Job list status filter and light auto-refresh (opt-in or when `durable_hook` is active; **stops on terminal**; no invented progress) are thinner SPA polish — not a SPA framework. Cancel confirm dialog: cancel ends the run (`canceled`) from running, paused, or held; pause exists on the durable/ctl path only. See [docs/ux-side-by-side.md](docs/ux-side-by-side.md). This does **not** close #70.
