...
- `GET /v0/jobs/{id}/compare` — thinner vs recent same-catalog (or same kind/class) jobs already in this process. Elapsed for this run + priors from created/updated timestamps; typical/ETA only from succeeded prior walls when two or more samples exist. **not a forecast**, **not** IFRS17, **not** iec SPA historical widget. No guest→ctl HTTP.
