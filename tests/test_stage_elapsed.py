...
class ElapsedPureTests(unittest.TestCase):
    ...
    def test_dry_run_wall_plan_omits(self) -> None:
        ...
        self.assertIs(plan["north_star_done"], False)
        self.assertEqual(plan["runtime_tip"], "9b6646e8")
        self.assertEqual(plan["docs_tip"], "6511cec7")
        self.assertEqual(plan["or"], "main")
        self.assertEqual(plan["runtime_pr"], 114)
        self.assertEqual(plan["docs_pr"], 116)
        self.assertIn("9b6646e8", plan["note"])
        self.assertIn("6511cec7", plan["note"])
        self.assertIn("compare", plan["note"].lower())
        self.assertIn("Never invent", plan["note"])
        self.assertIn("Omit when missing", plan["note"])
