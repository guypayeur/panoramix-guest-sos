...
class HttpAppTests(unittest.TestCase):
    ...
    def test_health_and_info(self) -> None:
        ...
        self.assertIn("ctl_admit_timeout", ctl["note"])
        self.assertIn("#143", ctl["note"])
        ...
        self.assertIn("ctl_admit_timeout", body["jobs"]["ctl_admit_timeout"])
        self.assertIn("runtime #143", body["jobs"]["ctl_admit_timeout"])
        self.assertIn("mid-flight", body["jobs"]["ctl_admit_timeout"])
        self.assertIn("no stub progress", body["jobs"]["ctl_admit_timeout"])
        ...
        self.assertIn("not iec SPA", body["jobs"]["progress_honesty"])
    ...
    def test_ui_pages(self) -> None:
        for path in ("/", "/ui"):
            ...
            self.assertIn("wall_elapsed_ms", html)
            self.assertIn("durableWallMs", html)
            self.assertIn("durable wall", html)
            self.assertIn("not a forecast", html)
            self.assertIn("not IFRS17", html)
            self.assertIn("not iec SPA", html)
            ...
            self.assertIn("runtime #143", html)
            self.assertIn("mid-flight", html)
