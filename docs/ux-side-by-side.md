...
- Job detail for a selected run (identity, timestamps, current message/stage, stub vs runtime `backed`). Progress prefers durable path-slice counters when a hook provides them; otherwise stub “stage i of n”. Events prefer durable JSONL when a hook provides them; otherwise process-memory. Investigate (thinner): catalog name + short digest called out for cross-check; static path-slice ownership tags when hooked; event trail on the same panel.
- Cancel of a live run with clear framing and `canceled` (one L). Stub-backed cancel is **guest local**. Durable temporal-local cancel/status/pause/resume is **operator/ctl** (`runtime.apply reserve-temporal`), not guest→ctl HTTP. An injected hook (if present) is signaled first, then the guest job is marked `canceled` if still live (running or paused). Terminal cancel is **409**. The default hook stays inert. Opt-in lab (`PANORAMIX_RUNTIME_ROOT`) may subprocess `reserve-temporal` on that same hook seam. Cancel is not pause.
...
- Opaque WorkHandoff emit (`kind` / `class` / `payload_digest` + `id` / `status`) and payload export for the ctl path. Guest does **not** call `runtime.apply compute-work`. Default guest does not invoke apply; opt-in lab may subprocess `reserve-temporal` only.
...
- **Pause / resume full Temporal-backed UX.** Guest HTTP/UI pause/resume is thinner: durable-backed only (injected hook or opt-in lab adapter), stub refused honestly. Default hook stays inert — no guest→ctl HTTP. Operators still use `python3 -m runtime.apply reserve-temporal pause|resume --id cw_…` for the ctl path, or set `PANORAMIX_RUNTIME_ROOT` for local-lab subprocess on the same hook. This does **not** stamp the #70 UX box.
- **Temporal-backed guest UX.** Durable admit/status/cancel/pause/resume is operator/ctl (`reserve-temporal`). Guest UI cancel remains local stub cancel unless a hook admits. Opt-in lab loopback is a thinner local subprocess on the existing hook seam — not #70 Done, not cloud, not IFRS17.
...
- **SPA polish**: no historical-run comparison, no ETA vs prior quarter, no Slack, no live team directory, no data-catalog product. Guest investigate is catalog identity + static ownership tags on durable path-slices + event trail — **not** iec SPA / Slack / real data catalog.
...
- [ ] Pause / resume only when a real compute-plane durable path exists (not a guest stub) — guest HTTP is thinner/durable-only; default hook still inert; ctl `reserve-temporal pause|resume` is the operator path; opt-in lab adapter is the same hook seam, not a stub pretend
- [ ] Temporal-backed UX (guest UI status/cancel/pause/resume follows durable temporal-local runs without an injected hook) — operator/ctl `reserve-temporal` exists; opt-in lab still uses the hook injection point; default remains inert stub cancel + disabled stub pause
