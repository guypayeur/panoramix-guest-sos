...
| Durable ctl (temporal-local) | operator/ctl `runtime.apply reserve-temporal` admit\|status\|progress\|cancel\|pause\|resume on panoramix-runtime **main** (`progress --id` → `stage` / `stages_total` / `stages_completed` / `fraction` / nested `progress`; status `paused`) + runtime [`bindings/local-reserve-temporal.example.yaml`](https://github.com/guypayeur/panoramix-runtime/blob/main/bindings/local-reserve-temporal.example.yaml) (pin 0.5; verified @ `63c4d8e`) |
...
- Job detail for a selected run (identity, timestamps, current message/stage, stub vs runtime `backed`). Progress prefers durable path-slice counters when a hook provides them; otherwise stub “stage i of n”.
