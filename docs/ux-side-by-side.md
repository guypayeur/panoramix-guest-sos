...
- Opaque WorkHandoff emit (`kind` / `class` / `payload_digest` + `id` / `status`) and payload export for the ctl path. Failed/canceled recoverability points at the same exports for operator/ctl `reserve-temporal admit --handoff JSON`. Guest does **not** call `runtime.apply compute-work`. Default guest does not invoke apply; opt-in lab may subprocess `reserve-temporal` only.
