...
| Durable ctl (temporal-local) | operator/ctl `runtime.apply reserve-temporal` admit\|status\|progress\|events\|cancel\|pause\|resume on panoramix-runtime **main** (`progress --id` → `stage` / `stages_total` / `stages_completed` / `fraction` / nested `progress`; `events --id` → JSONL under `.runtime/reserve-temporal/events/`; status `paused` + `events` / `events_n` / `events_durable`) + runtime [`bindings/local-reserve-temporal.example.yaml`](https://github.com/guypayeur/panoramix-runtime/blob/main/bindings/local-reserve-temporal.example.yaml) (pin 0.5; verified @ `63c4d8e`) |
