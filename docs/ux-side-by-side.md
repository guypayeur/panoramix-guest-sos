...
| 2.1 Pause | `POST /v1/jobs/{id}/pause` | **missing** | No pause. Do not pretend Temporal exists |
...
| 3.1b Cancel | `POST /v1/jobs/{id}/cancel` | **match** (thinner) | Confirm dialog states: no pause/resume; Cancel ends the run (`canceled`); stub vs runtime-backed cancel in one sentence |
...
- Cancel of a live run with clear framing and `canceled` (one L). Stub-backed cancel is local; runtime-backed cancel signals an injected hook, then marks the guest job `canceled` if still live. Terminal cancel is **409**.
...
- **Pause / resume** (iec Temporal-signal path). This guest will not add buttons that pretend durable orchestration exists.
