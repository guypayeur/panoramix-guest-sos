...
This is the guest follow-up for runtime [#146](https://github.com/guypayeur/panoramix-runtime/issues/146) / [PR #148](https://github.com/guypayeur/panoramix-runtime/pull/148) so [#70](https://github.com/guypayeur/panoramix-runtime/issues/70) north-star compare can admit the **same** `reserve_ifrs17` job iec measures. Same-job identity now matches iec method tip [`89fd962`](https://github.com/guypayeur/iec-proto-c/commit/89fd962f5cebdd7033c887a05a382032da5560cd) ([iec-proto-c #27](https://github.com/guypayeur/iec-proto-c/pull/27) / [#28](https://github.com/guypayeur/iec-proto-c/pull/28); lockstep with runtime **main** @ [`0baa354`](https://github.com/guypayeur/panoramix-runtime/commit/0baa3544e97ebde170e42ef172e69786360c8fad) / [#161](https://github.com/guypayeur/panoramix-runtime/pull/161) GPU class; `docs/iec-local.md`): pin `89fd962…`, digest `sha256:21c32109…`. Sibling reserve-temporal compose stays [`docs/lab-compose.md`](lab-compose.md) (ctl **19215**).
...
Alias `same-job` → `reserve_ifrs17`. Digest must match runtime `reserve_iec.SAME_JOB_DIGEST` for pin `89fd962` (canonical four-field identity; lockstep with runtime **main** @ `0baa354`, [#161](https://github.com/guypayeur/panoramix-runtime/pull/161)):
...
```json
{"mode":"STANDARD","revision":"89fd962f5cebdd7033c887a05a382032da5560cd","source_file":"reserve_ifrs17/reserve_ifrs17.adsl","workload":"reserve_ifrs17"}
```

Prior same-job `api_e2e` walls stay cited at pin `4d5d44d` (pack `iec-parity-same-job-20260912-b`). Pin `69b1b1d` **enabled held UX** (GET `is_paused` / `PAUSED`; ctl `can_pause` / `can_resume`). Pin `89fd962` is iec [#27](https://github.com/guypayeur/iec-proto-c/pull/27) / [#28](https://github.com/guypayeur/iec-proto-c/pull/28) (Platform GPU + `N_ACCOUNTS` resolution). Default same-job shape remains **40×100** (accounts × scenarios). Do **not** bake 200k as the default. It does **not** invent new comparable walls.

This is **not** the thinner recorded / live / parity kernel. `class: gpu` is refused on the guest POST (runtime [#161](https://github.com/guypayeur/panoramix-runtime/pull/161) GPU class lives on the binding). Thinner reserve ints (`accounts`, `horizon`, …) are refused. Without a durable hook the guest **fail-closes** (`durable_admit_failed` / `same_job_stub`) — no in-guest IFRS17 stub.

## What it starts

1. `python3 -m runtime.serve --binding bindings/local-iec.example.yaml` from a [panoramix-runtime](https://github.com/guypayeur/panoramix-runtime) checkout (**main** @ `0baa354`, [#161](https://github.com/guypayeur/panoramix-runtime/pull/161); method pin lockstep `89fd962`). Binding `publish.ctl_port` is **19216**.
...
Ctl verbs (runtime `docs/iec-local.md` @ `0baa354`):
