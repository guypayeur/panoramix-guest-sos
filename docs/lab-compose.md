...
3. `POST /v0/jobs` with `{"demo":"reserve","catalog":"recorded"}` by default. Opt-in `--catalog live|parity` (alias `parity-scale`, or `LAB_COMPOSE_CATALOG`) selects that catalog. Default remains recorded so CI / `--dry-run` stay unchanged. Parity digest is `sha256:e180d2c2e3589b8762f92efa1bedb3d53ffeeb16648581ba13d537bcd3311102` (guest already mirrors `runtime.reserve.parity_params` / `digest_for`). Parity ≠ IFRS17 / ≠ ADSL. Does **not** stamp `north_star_done`. Does **not** unlock cloud. Does **not** invent walls or flip comparison flags.
