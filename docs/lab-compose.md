...
3. `POST /v0/jobs` with `{"demo":"reserve","catalog":"recorded"}` by default. Opt-in `--catalog live|parity` (alias `parity-scale`, or `LAB_COMPOSE_CATALOG`) selects that catalog. Default remains recorded so CI / `--dry-run` stay unchanged. Parity digest is `sha256:e180d2c2e3589b8762f92efa1bedb3d53ffeeb16648581ba13d537bcd3311102` (guest already mirrors `runtime.reserve.parity_params` / `digest_for`). Parity is minutes-class — admit must return a running `cw_…` id (runtime [#143](https://github.com/guypayeur/panoramix-runtime/issues/143)) so the UI can poll mid-flight; compose does **not** wait-for-succeed. Parity ≠ IFRS17 / ≠ ADSL. Does **not** stamp `north_star_done`. Does **not** unlock cloud. Does **not** invent walls or flip comparison flags.
...
CI uses `--dry-run` (plan JSON plus in-process re-admit smokes: hooked new job id, inert / payload-unknown **409** `re_admit_unavailable`). Plan also records `async_admit` (runtime #143; `ctl_admit_timeout`; mid-flight poll; `invent: false`; `north_star_done` false; live|parity `stub_fallback` false), `timeline_elapsed` (runtime tip `9ba95bbb` / docs tip `5dc191cb` / main; `omit_when_missing: true`; `invent: false`), `wall_elapsed` (runtime tip `9b6646e8` / PR #114 / main; docs tip `6511cec7` / PR #116; `omit_when_missing: true`; `invent: false`; compare prefers when present), `handoff_docs` (panel; not a second control plane), and `pack_fill` (paste fragment for runtime #78 D; documented runtime tip `b81130f` / PR #120 / main; docs tip `63a168d` / PR #122 WSL assist-smoke stamp lineage; gap-report tip `84cb202` / PR #124; gap-report docs tip `aa4f09e` / PR #126 / main; merge tip `5086d0f` / PR #128 / main; merge docs tip `6ecb645` / PR #130 / main; apply-metrics tip `5d399f7` / PR #132 / main; apply-metrics docs tip `3904ee4` / PR #134 / main; apply-notes tip `48a8645` / PR #136 / main; apply-notes docs tip `6807509` / PR #138 / main; guest emit tip `b859466` / PR #52; guest gap-report tip `eb48605` / PR #60; guest merge tip `39064d5` / PR #64; guest apply-metrics tip `7c09f32` / PR #68; guest apply-notes tip `e5562c3` / PR #70; emit → `runtime.iec_parity_pack skeleton` via `--from-json` / flags; `pack_fill.apply_notes` hint for `runtime.iec_parity_pack apply-notes …` (does **not** supply wall numbers; does **not** invent UX strings); `pack_fill.apply_metrics` hint for `runtime.iec_parity_pack apply-metrics … --from-durable-panoramix` (does **not** supply wall numbers); `pack_fill.merge` hint for `runtime.iec_parity_pack merge … --from-json`; `pack_fill.gap_report` hint for `gap-report` — cd+cmd when `PANORAMIX_RUNTIME_ROOT` known, else a copy-paste template; does **not** run apply-notes, apply-metrics, merge, or gap-report; wall feature tip remains `9b6646e8`; `omit_when_missing: true`; `invent: false`; `invent_wall_time_sec: false`; `writes_live_pack: false`; assist ≠ fill; apply-notes ≠ fill; apply-notes ≠ Done; apply-metrics ≠ fill; apply-metrics ≠ Done; merge ≠ fill; merge ≠ Done; assist ≠ Done; gap-report ≠ Done; durable omitted on dry-run because nothing was measured). No live Temporal is required for unit tests. Fail-closed without hook or payload — **no silent stub**.

## Commands

```bash
# Plan + in-process re-admit smokes (no processes, no Temporal).
# Default catalog recorded — CI / --dry-run unchanged:
python3 scripts/lab_compose_reserve_temporal.py --dry-run

# Inspect a non-default POST body without Temporal (smokes stay recorded):
python3 scripts/lab_compose_reserve_temporal.py --dry-run --catalog parity
...
# Opt-in parity catalog for a #78 D comparable-scale admit
# (minutes-class: mid-flight poll + cancel, not wait-for-succeed;
# needs runtime #143 admit-return-running; timeout is ctl_admit_timeout;
# not IFRS17/ADSL; does not stamp north_star_done; does not unlock #61/#29;
# does not invent walls or flip comparison flags):
python3 scripts/lab_compose_reserve_temporal.py --catalog parity
# or: export LAB_COMPOSE_CATALOG=parity
```
...
# terminal 3 — recorded reserve (default), then cancel + one-click re-admit.
# Opt-in live|parity (alias parity-scale) the same way:
...
When the env is set to a valid loopback origin but `runtime.serve` is down (connection refused), admit/status/progress fail closed with **`ctl_http_unreachable`** (lab-serve down) — not a hung poll, not pretend durable. Start serve or unset the env. Not #70 Done.

Admit that exceeds guest timeout (HTTP admit 8s / poll 1.5s / ctl-apply admit 2s) before a running id is **`ctl_admit_timeout`** — not lab-serve-down. HTTP timeout while the origin is listening is not lab-serve-down. live|parity is minutes-class and depends on runtime [#143](https://github.com/guypayeur/panoramix-runtime/issues/143) so admit returns `{id: cw_…, handoff.status: running}` while work continues. Guest then polls `GET /v0/jobs/{id}/progress` and `GET /events` mid-flight. Fail closed — **no stub progress**. Recorded stays sync-fast. Not #70 Done. `north_star_done` false.
...
python3 -m runtime.iec_parity_pack apply-notes ~/panoramix-lab/evidence-70/iec-parity/iec-parity.json
...
# pack_fill.apply_notes.hint is:
# cd $PANORAMIX_RUNTIME_ROOT && python3 -m runtime.iec_parity_pack apply-notes \
...
**apply-notes ≠ fill; apply-notes ≠ Done; apply-metrics ≠ fill; apply-metrics ≠ Done; assist ≠ Done; merge ≠ fill; merge ≠ Done; gap-report ≠ Done.** The hints do **not** run apply-notes, apply-metrics, merge, or gap-report (CI `--dry-run` stays recorded; no live pack required). The apply-metrics hint does **not** supply wall numbers. The apply-notes hint does **not** supply wall numbers or invent UX strings. Null walls stay null. Does **not** invent `metrics.wall_time_sec`. Does **not** stamp `north_star_done`. Apply-notes tip `48a8645` / [PR #136](https://github.com/guypayeur/panoramix-runtime/pull/136) or **main**; apply-notes docs stamp `6807509` / [PR #138](https://github.com/guypayeur/panoramix-runtime/pull/138) or **main**; apply-metrics tip `5d399f7` / [PR #132](https://github.com/guypayeur/panoramix-runtime/pull/132) or **main**; apply-metrics docs stamp `3904ee4` / [PR #134](https://github.com/guypayeur/panoramix-runtime/pull/134) or **main**; merge tip `5086d0f` / [PR #128](https://github.com/guypayeur/panoramix-runtime/pull/128) or **main**; merge docs stamp `6ecb645` / [PR #130](https://github.com/guypayeur/panoramix-runtime/pull/130) or **main**; gap-report tip remains `84cb202` / [PR #124](https://github.com/guypayeur/panoramix-runtime/pull/124); docs stamp `aa4f09e` / [PR #126](https://github.com/guypayeur/panoramix-runtime/pull/126) or **main**; wall tip remains `9b6646e8`. WSL stamp `~/panoramix-lab/evidence-70/stamp-5086d0f-iec-parity-merge/` is assist smoke only (assist ≠ fill). WSL stamp `~/panoramix-lab/evidence-70/stamp-5d399f7-iec-parity-apply-metrics/` is assist smoke only (assist smoke ≠ fill; smoke walls ≠ Done). WSL stamp `~/panoramix-lab/evidence-70/stamp-48a8645-iec-parity-apply-notes/` is assist smoke only (assist smoke ≠ fill; smoke notes ≠ Done). Operator live #78 D pack is off-box at `~/panoramix-lab/evidence-70/iec-parity-live-20260912/` with STRICT `stamp-48a8645-iec-parity-live/` — **live fill ≠ Done**; `north_star_done` false; `comparable` false; does **not** unlock cloud.
...
- Claiming opt-in `live|parity` (alias `parity-scale`) is IFRS17 / ADSL
- Claiming #70 UX Done or `north_star_done`
- Cloud unlock (#61 / #29)
- Closing runtime #70 / #78
- Claiming runtime #143 is closed from this guest
- Inventing walls or flipping comparison flags because the catalog changed
- Pretend durable / stub progress when live|parity admit times out
...
- Claiming `runtime.iec_parity_pack apply-notes` is a fill / Done
- Claiming the off-box live pack (`iec-parity-live-20260912` / STRICT `stamp-48a8645-iec-parity-live`) is Done / `north_star_done` / a cloud unlock
